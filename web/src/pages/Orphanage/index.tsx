...
export default function Orphanage() {
  ...
  useEffect(() => {
    api.get(`orphanages/${id}`).then((resp) => {
      const data: IOrphanage = resp.data;

      setOrphanage(data);
    });
  }, [id]);

  // Handles orphanage not loaded
  if (!orphanage) {
    ...
    <div id="page-orphanage">
      ...
      <main>
        <div className="orphanage-details">
          {orphanage.images[actualImage] && (
            <>
              <img
                src={orphanage.images[actualImage].path}
                alt={orphanage.name}
              />
              ...
              <div className="images">
                {orphanage?.images.map((image, index) => (
                  <button
                    type="button"
                    key={image.id}
                    className={index === actualImage ? "active" : ""}
